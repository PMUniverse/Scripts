    ...
    public class PitchBlackAbyss {
        	...
        	for (int i = 0; i < num; i++) {
        		...
        		if (imap.Tile[randx,randy].Type == Enums.TileType.Walkable) {
        			...
        			if (rand < 31) { //healing item
        				...
        				rand = Server.Math.Rand(0,10);
        				if (rand < 6) itemnum = treasure[0];
        				else if (rand < 10) itemnum = treasure[1];
        				else itemnum = treasure[2];
        			}
        			#endregion
        ...
        public static Room GetRoomType(Client client) {
        	...
        	try {
                ...
                Messenger.AdminMsg("X: " + exPlayer.Get(client).DungeonX.ToString(), Text.Black);
                Messenger.AdminMsg("Y: " + exPlayer.Get(client).DungeonY.ToString(), Text.Black);
